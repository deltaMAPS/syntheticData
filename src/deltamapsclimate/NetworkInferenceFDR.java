 ...
 * @author Foudalis
 */
public class NetworkInferenceFDR {
    ...
    public NetworkInferenceFDR(DeltaMapsClimate parent, Vector<Area> areas, File matFile)
    {        
        this.areas = areas;
        ...
        double minPval = fdr(1);
        networkInference(minPval);
        ...
        //exportSijkMap(areas.firstElement());
    ...
    /*
    type = 0, set c(v) = 1. type = 1, set c(v) = \sum_{i=1}^M 1/i, M being the total number of p-values.
    */
    private double fdr(int type)
    {
        ...
        for(int i = 0; i < pvalues.size(); i++)
        {
            ...
            {
                double cv  = Math.log(M)+0.5772;
                fdrRatio = (this.fdrQ*k)/(M*cv);
            }
            if(pval > fdrRatio)
            {
    ...
    private void networkInference(double minPval)
    {
